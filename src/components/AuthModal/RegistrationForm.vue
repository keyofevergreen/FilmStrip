  ...
  <form class="form-wrap" @submit.prevent novalidate>
    ...
    <div class="btn-wrap">
      <a-spin :spinning="isFetching">
        <button class="btn btn-registration" @click="handleSuccess">
          Зарегистрироваться
        </button>
      </a-spin>
    </div>
...
export default {
  ...
  computed: {
    ...
    userMailErrors() {
      ...
      if (this.v$.form.userMail.hasUnregisteredMail.$invalid)
    ...
    userPhoneNumberErrors() {
      if (this.v$.form.userPhoneNumber.required.$invalid)
        return '* Обязательно для заполнения.';
      if (this.v$.form.userPhoneNumber.hasValidPhoneNumber.$invalid)
        return '* Неправильный формат номера.';
      if (this.v$.form.userPhoneNumber.hasUnregisteredPhoneNumber.$invalid)
        return '* Аккаунт с таким номером уже существует.';
      return null;
    },
    userPasswordErrors() {
      if (this.v$.form.userPassword.required.$invalid)
        return '* Обязательно для заполнения.';
      if (this.v$.form.userPassword.minLength.$invalid)
  ...
  methods: {
    ...
    createAccount() {
      ...
      const user = {
        ...
        phoneNumber: this.form.userPhoneNumber.replace(/[^0-9]/g, ''),
